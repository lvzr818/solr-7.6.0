 ...
 * ...
 */
public class ReRankingHandler20220407 extends SearchHandler /*RequestHandlerBase implements PluginInfoInitialized*/ {
...
//				final SearchHandler searchHandler = (SearchHandler) req.getCore().getRequestHandler("/select");
		...
		this.API_CONSUMER_MAP.get(req.getPath()).accept(req, rsp);
	}
	...
	private final BiConsumerPlus<SolrQueryRequest, SolrQueryResponse> handleRequestBody = (req, rsp) -> {
		...
		if (!(params instanceof MultiMapSolrParams)) {
			ReRankingHandler20220407.this.invokeSuperHandleRequestBody(req, rsp);
			return;
		}
		if (ReRankingHandler20220407.this.semaphore.tryAcquire(2)) {
			ReRankingHandler20220407.this.executor.submit(ReRankingHandler20220407.this.runnable);
		...
		final int originalRows = Optional.ofNullable(params.getInt(CommonParams.ROWS)).orElse(ReRankingHandler20220407.this.defaultPageSize);
		...
		if (ReRankingHandler20220407.this.limitPageNo < index + 1) {
			ReRankingHandler20220407.this.invokeSuperHandleRequestBody(req, rsp);
			return;
		}
		...
		while (true) {
			//  缓存未写入或者写入失败
			if (ReRankingHandler20220407.this.FAILED_WRITE_TO_CACHE.get() || originalStart >= ReRankingHandler20220407.this.numFound) {
				if (ReRankingHandler20220407.this.numFound != 0) {
					ReRankingHandler20220407.this.invokeSuperHandleRequestBody(req, rsp);
					return;
				}
			}
			final int pageTabSize = ReRankingHandler20220407.this._CACHE_PAGE_TAB.size();
			final int nowTotalNum = ReRankingHandler20220407.this._CACHE_ALL_SERVICE_IDS.size();
			//  缓存数据量不足~1
			if ((index >= pageTabSize || atLeastNeed > nowTotalNum) && ReRankingHandler20220407.this.numFound != nowTotalNum) {
			...
			if (ReRankingHandler20220407.this.defaultPageSize == originalRows && index < pageTabSize && originalStart % originalRows == 0) {
				final List<String> pageContent = ReRankingHandler20220407.this._CACHE_PAGE_TAB.get(index);
				//  TODO:LVZR step-1
			...
			paramsMap.put(CommonParams.Q, new String[]{String.format("%s:(%s)", ReRankingHandler20220407.this.SERVICE_ID,
					tmp.substring(1, tmp.length() - 1).replace(",", " "))});
			req.setParams(new MultiMapSolrParams(paramsMap));
			ReRankingHandler20220407.this.invokeSuperHandleRequestBody(req, rsp);
			MultiMapSolrParams.addParam("num_found", String.valueOf(ReRankingHandler20220407.this.numFound), paramsMap);
			MultiMapSolrParams.addParam(CommonParams.WT, ReRankingHandler20220407.this.wt, paramsMap);
			req.setParams(new MultiMapSolrParams(paramsMap));
			return;
		}
	};
